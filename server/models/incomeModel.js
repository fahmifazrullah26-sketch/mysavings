...
const IncomeModel = {
  async create(userId, { transactionDate, amount, category, description }) {
    const [result] = await pool.execute(
      `INSERT INTO income (user_id, transaction_date, amount, category, description)
       VALUES (?, ?, ?, ?, ?)`,
  ...
  async findById(id, userId) {
    const [rows] = await pool.execute(`SELECT * FROM income WHERE id = ? AND user_id = ?`, [id, userId]);
    return rows[0];
  },

  async update(id, userId, { transactionDate, amount, category, description }) {
    await pool.execute(
      `UPDATE income SET transaction_date = ?, amount = ?, category = ?, description = ?
  ...
  async remove(id, userId) {
    await pool.execute(`DELETE FROM income WHERE id = ? AND user_id = ?`, [id, userId]);
  },

  async sumByUser(userId, { startDate, endDate } = {}) {
    let sql = `SELECT COALESCE(SUM(amount), 0) AS total FROM income WHERE user_id = ?`;
    const params = [userId];
  ...
  async monthlyTotals(userId, year) {
    const [rows] = await pool.execute(
      `SELECT MONTH(transaction_date) AS month, SUM(amount) AS total
       FROM income WHERE user_id = ? AND YEAR(transaction_date) = ?
